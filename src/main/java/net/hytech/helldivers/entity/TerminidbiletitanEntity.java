...
public class TerminidbiletitanEntity extends PathfinderMob implements RangedAttackMob, GeoEntity {
	...
	public class RangedAttackGoal extends Goal {
		...
		public void stop() {
			...
			this.seeTime = 0;
			this.attackTime = -1;
			((TerminidbiletitanEntity) rangedAttackMob).entityData.set(SHOOT, false);
		}
		...
		public void tick() {
			...
			if (--this.attackTime == 0) {
				...
				((TerminidbiletitanEntity) rangedAttackMob).entityData.set(SHOOT, true);
	...
	public void performRangedAttack(LivingEntity target, float flval) {
		BileSpewProjectileEntity.shoot(this, target);
	}
	...
	public static void init() {
		SpawnPlacements.register(HelldiversModEntities.TERMINIDBILETITAN.get(), SpawnPlacements.Type.NO_RESTRICTIONS, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, Mob::checkMobSpawnRules);
	}
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F)) && !this.isVehicle()) {
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.bile_titan.walk"));
			}
	...
	private PlayState attackingPredicate(AnimationState event) {
		...
		if ((this.swinging || this.entityData.get(SHOOT)) && event.getController().getAnimationState() == AnimationController.State.STOPPED) {
			...
			return event.setAndContinue(RawAnimation.begin().thenPlay("animation.bile_titan.stomp"));
		}
